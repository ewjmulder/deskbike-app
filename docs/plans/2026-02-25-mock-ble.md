...
In the `## Commands` block, add after `pnpm emulator`:

```markdown
MOCK_BLE=1 pnpm dev  # Start with software BLE mock (no hardware needed)
```

**Step 2: Add mock explanation to BLE emulator section**

After the existing "Linux constraint" paragraph in `## BLE emulator`, add:
